...
$query = "SELECT * FROM posts WHERE id =$post_id AND post_status = 'Published' ";
    ...
    justify-content: center;
    align-content: center;
"><h4>Sorry: No Comment yet!!!</h4></div>';
			}
				}else{
					$output = "Failed to load resource: the server responded with a status of 503";
				}
...
if($_POST['action'] == 'fetch_mobileapps'){
			$query = "SELECT * FROM posts WHERE (post_tag LIKE '%Mobile%' OR category LIKE '%Mobile%') AND post_status = 'Published' ORDER BY id DESC LIMIT 3";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			if($send){
			foreach ($send as $row) {
...
if($_POST['action'] == 'fetch_posts_by_cat1'){
				...
				foreach ($send as $row) {
					...
					$query = "SELECT * FROM posts WHERE (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND post_status = 'Published' ORDER BY id DESC LIMIT 5";
				$send = $database->query($query);
				$row = $database->fetch_array($send);
				if($send){
					if($row >0){
				foreach ($send as $row) {
				  ...
				  echo '<div class="container jumbotron" style="
    display: flex;
    width: 100%;
    padding: 10px 20px;
    justify-content: center;
    align-content: center;
"><h3 align="center">SORRY...</br>No Posts yet!!!</h3></div>';}
				}else{
					$output = "Failed to load resource: the server responded with a status of 503";
				}
}
...
if($_POST['action'] == 'fetch_posts_by_cat2'){
				...
				foreach ($send as $row) {
					...
					$query = "SELECT * FROM posts WHERE (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND post_status = 'Published' ORDER BY id DESC LIMIT 40";
				$send = $database->query($query);
				$row = $database->fetch_array($send);
				$rowcount = $database->num_rows($send);
				if($send){
...
if($_POST['action'] == 'fetch_posts_by_cat3'){
			...
			foreach ($send as $row) {
				...
				$query = "SELECT * FROM posts WHERE (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND post_status = 'Published' ORDER BY id DESC LIMIT 3";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			if($send){
			if($row>0){
...
if($_POST['action'] == 'fetch_posts_by_cat5'){
 ...
 if($_POST['action'] == 'fetch_posts_by_cat6'){
			...
			foreach ($send as $row) {
				...
				$query = "SELECT * FROM posts WHERE (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND post_status = 'Published' ORDER BY id DESC LIMIT 4";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			if($send){
				if($row>0){
...
"><h3 align="center">SORRY...</br>No Posts yet!!!</h3></div>';
 ...
 if($_POST['action'] == 'fetch_posts_by_cat7'){
			...
			foreach ($send as $row) {
				...
				$query = "SELECT * FROM posts WHERE (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND post_status = 'Published' ORDER BY id DESC LIMIT 6";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			if($send){
			foreach ($send as $row) {
...
if($_POST['action'] == 'fetch_recentgadgets'){
			$query = "SELECT * FROM posts WHERE (post_tag LIKE '%Gadgets%' OR category LIKE '%Gadgets%') AND post_status = 'Published' ORDER BY id DESC LIMIT 10";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			if($send){
			foreach ($send as $row) {
...
if($_POST['action'] == 'fetch_footer_posts'){
					$query = "SELECT * FROM posts WHERE post_status = 'Published' ORDER BY post_views DESC LIMIT 10";
					$send = $database->query($query);
					$row = $database->fetch_array($send);
					foreach ($send as $row) {
						$post_id = $row['id'];
 ...
 if($_POST['action'] == 'fetch_navnews'){
				$query = "SELECT * FROM posts WHERE post_status = 'Published' AND (post_tag LIKE '%News%' OR category LIKE '%News%') ORDER BY id DESC LIMIT 5";
				$send = $database->query($query);
				$row = $database->fetch_array($send);
				foreach ($send as $row) {
					$post_id = $row['id'];
...
if($_POST['action'] == 'fetch_megatab_posts'){
			...
			if($row>0){
				...
				$menu = $row['menu1'];
					...
					$query = "SELECT * FROM posts WHERE post_status = 'Published' AND (post_tag LIKE '%".$menu."%' OR category LIKE '%".$menu."%') ORDER BY post_views DESC LIMIT 4";
					$send = $database->query($query);
					$row = $database->fetch_array($send);
					if($row >0){
					foreach ($send as $row) {
...
echo'<time class="published" datetime="'.$row['post_time'].'">'.$post_time.'</time></span></div></div></article>';
			    ...
			    echo '<div class="container-fliud jumbotron text-center" style="
    display: flex;
    width: 100%;
    padding: 10px 20px;
    justify-content: center;
...
if($_POST['action'] == 'fetch_breaking_news'){
			$query = "SELECT * FROM posts WHERE post_status = 'Published' AND (post_tag LIKE '%News%' OR category LIKE '%News%')  ORDER BY id DESC LIMIT 6";
			$send = $database->query($query);
			$row = $database->fetch_array($send);
			$output ="";
			if($row>0){
...
if($_POST['action'] == 'fetch_relatedposts'){
			...
			// count($post_tags)
					$query = "SELECT * FROM posts WHERE post_status = 'Published' AND (post_tag LIKE '%".$post_cat."%' OR category LIKE '%".$post_cat."%') AND id != $post_id ORDER BY id DESC LIMIT 3";
					$send = $database->query($query);
					$row = $database->fetch_array($send);
					$rowCount = $database->num_rows($send);
					if($rowCount > 0){
...
echo'<time class="published" datetime="'.$row['post_time'].'">'.$post_time.'</time></span></div></div></article>';
    ...
    justify-content: center;
    align-content: center;
"><h4 class="justify-content-center align-content-center">Sorry: No Suggestions yet!!!</h4></div>';
			}
