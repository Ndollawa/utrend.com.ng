...
$RPperInc= ceil((($monthlytrashedpost-$previousmonthTPost)/$previousmonthTPost)*100);
 ...
 $sql ="SELECT COUNT(id) as total FROM users WHERE user_acctStatus = 'Active' AND role ='Subscriber' ";
 $result =$database->query($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
...
$sql ="SELECT * FROM users WHERE user_acctStatus = 'Pending' AND role ='Subscriber'  AND YEAR(date_created)=YEAR(CURRENT_DATE ) AND MONTH(date_created) = MONTH(CURRENT_DATE)";
 $result= $db->query ($sql);
$monthlypendingusers  = $database->num_rows($result);
$sql="SELECT * FROM users WHERE user_acctStatus = 'Pending' AND role ='Subscriber'  AND date_created BETWEEN date_format(CURRENT_DATE - INTERVAL 1 MONTH, '%Y-%m-01') AND date_format(CURRENT_DATE , '%Y-%m-01')";
 $result= $db->query ($sql);
...
$row =$database->fetch_array($result);
 ...
 $sql="SELECT COUNT(id) as total FROM visitors WHERE date_visited BETWEEN date_format(CURRENT_DATE - INTERVAL 1 MONTH, '%Y-%m-01') AND date_format(CURRENT_DATE , '%Y-%m-01')";
 $result= $db->query ($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
 	$PMvisitors_count = $row['total'];
...
$sql="SELECT SUM(post_views) as total FROM posts WHERE post_status = 'Published'   AND post_time BETWEEN date_format(CURRENT_DATE - INTERVAL 1 MONTH, '%Y-%m-01') AND date_format(CURRENT_DATE , '%Y-%m-01')";
 ...
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
 	$PMengagement_count = $row['total'];
 } if($PMengagement_count>0){
$engagementperInc= ceil((($CMengagement_count-$PMengagement_count)/$PMengagement_count)*100);
}else{
$engagementperInc=0;	
}
 $sql ="SELECT COUNT(id) as total FROM users WHERE user_acctStatus = 'Deleted' ";
 $result =$database->query($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
 	$deleteduser_count = $row['total'];
 }
 $sql ="SELECT COUNT(id) as total FROM users WHERE user_acctStatus = 'Banned' ";
 $result =$database->query($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
 	$bannedusers_count = $row['total'];
 }

  $sql ="SELECT COUNT(id) as total FROM users WHERE account_type = 'Admin' AND user_acctStatus = 'Active' ";
 $result =$database->query($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
 	$adminusers_count = $row['total'];
 }
...
if ($online_status == "online") {
 ...
 $sql ="SELECT COUNT(id) as total FROM visitors";
 $result =$database->query($sql);
 $row=$database->fetch_array($result);
 foreach ($result as $row) {
