...
$sql ="SELECT COUNT(id) as total FROM comments WHERE post_id ='".$post_id."' AND comment_status != 'Deleted' ";
 ...
 foreach ($result as $row) {
	...
	$post_comments = $comment_count;							
					...
					<td>'.$post_time.'</td>
					<td style="display:inline-flex;">
						<a href="../source/content.php?source&recycledpost='.$post_id.'&title='.preg_replace("/ /","+", $post_title).'"  style="margin:2px 4px;" class="btn btn-primary btn-xs">
							<i class="fa fa-eye"></i>
						</a>
...
$sql ="SELECT COUNT(id) as total FROM comments WHERE post_id ='".$post_id."' AND comment_status != 'Deleted' ";
 ...
 foreach ($result as $row) {
  ...
  $output .= '<tr class="odd gradeX">
          ...
          <td>'.$post_time.'</td>
          <td style="display:inline-flex;">
            <a href="../source/content.php?source&p_id='.$post_id.'&title'.preg_replace("/ /", "+",$post_title).'" style="margin:2px 4px;" class="btn btn-success btn-xs" title="View Post">
              <i class="fa fa-eye"></i>
            </a>
