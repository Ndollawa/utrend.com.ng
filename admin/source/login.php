      ...
      $result= $database->query ($sql);
			...
			switch ($user_acctStatus) {
				case 'Deleted':			
					...
					break;
				case 'Banned':			
					$message = "<strong style='color:yellow'>SORRY: Your account has been banned!</strong>";
				...
				// 		$time = date('Y-m-d H:i:s');, last_activity = '$time'
						$sql="UPDATE sessions SET username = '$username', session_id = '".session_id()."' WHERE sessions.user_id = '".$_SESSION['user_id']."'";
						...
						if($user_account_type == "Admin" && $user_role == "Administrator"){
							...
							header ("location:../admin/profile.php");}	
							 }else { $message = "<div class='alert text-center' ><strong style='color:red'>Password incorrect!</strong></div>";}
										
					break;
			}

							 
							   }else{$message = "<div class='alert text-center' ><strong style='color:red'>Your email is incorrect!</strong></div>"; }
