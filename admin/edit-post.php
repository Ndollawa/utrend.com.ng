...
for ($i=0; $i <$count; $i++) {
	...
	$rowcount = $database->num_rows($send);
	if($rowcount>0){
	$sql = "UPDATE  post_attachments SET file_name = '$filename',file_type = '".$file_type[$i]."' WHERE post_id ='$post_id' AND file_index ='".$i."', ";
						...
						<div class="form-group row">
							<label class="control-label col-md-12">Post Content
								<span class="required"> * </span>										</label>
									<div class="col-md-12">
										<div class="card-body"> <textarea class="editor" name="post-content"><?php echo $post_content; ?></textarea> </div>
										</div></div>
											<div class="form-actions" style="margin-top:20px;">
								
									<div class="offset-md-5 col-md-9">
										<button type="submit" class="btn btn-info mr-20"  <?php if (isset($_SESSION['user_acctStatus']) && $_SESSION['user_acctStatus'] != 'Active'){
												echo 'disabled';} ?> name="updatepost-Submit">Update Post</button><button type="button" onClick = "history.back();"class="btn btn-default">Cancel</button>
