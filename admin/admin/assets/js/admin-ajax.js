...
var action = 'create-category';
 ...
 $(document).on('change','#account_type',(e)=>{
     let type = $('#account_type').val();
    //  alert(type);
     if(type === "Admin" ){ $('#account_role').html('<option value="Administrator">Administrator</option><option value="Moderator">	Moderator</option><option value="Editor">	Editor</option>') }else{ $('#account_role').html('<option value="Author">Author</option>');}
 });
...
$(document).on('submit', '#deletedpostdisTable', function(event) {
  event.preventDefault();
 bulk_options = $('#bulk-options').val();
 selectid =  $('.checkboxes:checked').map(function(){
   return $(this).val();
  ...
  selectid=JSON.stringify(selectid);
//   alert(selectid);
 var action = 'deletedPostsBulkOperation';
 if(bulk_options !== "" && selectid !== ""){
  $.ajax({
    url: 'admin-queries/table-queries.php',
    method: 'post',
...
function doSomething(scrollPos) {
    ...
    if(scrollPos){setTimeout(
        update_last_activity(),500);}
}
...
function welcomeUser(){
    ...
    $.ajax({
        ...
        success:function(data){
		  ...
		  if(script == 'login.php'){
		  	  $.toast({
		                                            heading: 'Welcome back! '+data[1] ,
		                                            text:'Last seen: '+data[0],
		                                            position: 'top-right',
		                                            loaderBg:'#ff6849',
		                                            icon: 'success',
		                                            hideAfter: 3500,
